...
class VaultRestRequestCreatorTestSuite extends FlatSpec with TestData with BaseTestSuite {
  ...
  "createTokenCreateRequest" should "create request which return token" in {
    ...
    val expectedData = getTokenInitParametersJson(useDefaultPolicy = false, policyName, period)
    ...
    val tokenInitParameters = TokenInitParameters(noDefaultPolicy = false, List(policyName), period)
  ...
  "createTokenCreateRequest" should "if response status is not equal to expected status, " +
    ...
    val tokenInitParameters = TokenInitParameters(noDefaultPolicy = false, List("name"), 1000)
