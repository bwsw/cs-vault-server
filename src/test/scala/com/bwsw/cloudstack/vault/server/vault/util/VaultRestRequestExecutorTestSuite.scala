...
class VaultRestRequestExecutorTestSuite extends FlatSpec with TestData with PrivateMethodTester with BaseTestSuite {
  ...
  "executeTokenCreateRequest" should "create request which returns token" in {
    ...
    val resultTokenResponse = vaultRestRequestCreator.executeTokenCreateRequest(testMapper.serialize(tokenInitParameters))
  ...
  "executeTokenCreateRequest" should "throw VaultFatalException if response status is not equal to expected status" in {
    ...
    assertThrows[VaultFatalException] {
      vaultRestRequestCreator.executeTokenCreateRequest(testMapper.serialize(tokenInitParameters))
    }
  }
