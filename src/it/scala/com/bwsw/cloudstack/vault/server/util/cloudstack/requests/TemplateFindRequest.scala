...
class TemplateFindRequest extends Request {
  override val request = new ApacheCloudStackRequest(RequestCommands.LIST_TEMPLATES)
    .addParameter(RequestParamaterKeys.RESPONSE, RequestParameterValues.JSON)
    .addParameter(RequestParamaterKeys.LIST_ALL, true)
    .addParameter(RequestParamaterKeys.TEMPLATE_FILTER, RequestParameterValues.FEATURED)
}
