...
docker run --cap-add IPC_LOCK -e VAULT_DEV_ROOT_TOKEN_ID="${IT_VAULT_ROOT_TOKEN}" \
                              ...
                              -p $IT_VAULT_PORT:$IT_VAULT_PORT --rm -d --name $VAULT_NAME vault:$VAULT_VERSION

#KAFKA
KAFKA_ACKS=all
KAFKA_WRITE_RETRIES=1

docker run -d --rm --name $KAFKA_NAME --tty=true -p $IT_ZOOKEEPER_PORT:$IT_ZOOKEEPER_PORT -p $IT_KAFKA_PORT:$IT_KAFKA_PORT --env ADVERTISED_HOST=$IT_KAFKA_HOST --env ADVERTISED_PORT=$IT_KAFKA_PORT spotify/kafka
...
docker run --rm -e KAFKA_HOST="${IT_KAFKA_HOST}" \
                ...
                --name $CLOUDSTACK_NAME -d -p $IT_CS_PORT:$IT_CS_PORT bwsw/cs-simulator-kafka:4.10.3-NP
